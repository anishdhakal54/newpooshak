                ...
                <table class="table table-striped">
                    ...
                    @foreach($products = $order->products as $product)
                        @php
                            ...
                            $side_count = intval($product->pivot->front)+intval($product->pivot->back)+intval($product->pivot->pocket);
                                  $color_price_ += $product->pivot->color_no * ($side_count) * getPerColorPrice() * $product->pivot->qty;
                                  $frameTotal+=getFrameRate($product->pivot->qty, $product->pivot->color_no);
                                  $colorTotal+=$product->pivot->total_color_price;

                        $discount = getDiscount($product->pivot->qty);
                        $discount_ +=$actualPrice * $discount / 100;
                        @endphp
                        ...
                        <tr>
                            ...
                            <td>{{ $product->pivot->qty }}<br>
                                ...
                                3XL: {{$product->pivot->quantity_3xl}}<br>
            ...
            <div class="col-xs-4">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            <th>Color price:</th>
                            <td>RS {{ number_format($color_price_, 2) }}</td>
                        </tr>
                        <tr>
                            <th>Frame Price:</th>
