...
//        dd($this->quantity_l);
    ...
    if($this->shipping_amount==null || $this->shipping_amount==0){
      $notify = json_notification('error', 'Shipping Address Error', 'Please dragon the pointer to your delivery address', 'linecons-pen');
...
//        Mail::to('nshdhkl@gmail.com')->send(new OrderShipped($data));
...
//        dd($this->rewards);
    ...
    if ($cartContents->count() > 0) {
      ...
      foreach ($cartContents as $cartContent) {
        ...
        $product->quantity = $product->quantity - $cartContent->stock_qty;
        $product->save();
